...
local function reload_workspace(buffer_number) -- 'bufnr' deve significar 'buffer_number' ou coisa parecida...
  ...
  for _, client in ipairs(clients) do
    ...
    client:request("rust_analyzer/reloadWorkspace", nil, function(any_error)
      if any_error then
        error(tostring(any_error))
      end
      vim.notify("Cargo workapace reloaded")
    end, 0)
  end
end
...
vim.lsp.config("rust_analyzer", {
    ...
    vim.system(cmd, { text = true }, function(output)
      if output.code == 0 then
        if output.stdout then
          ...
          if result["workapace_root"] then
            cargo_workspace_root = vim.fs.normalize(result["workapace_root"])
          end
        end
  ...
  before_init = function(init_params, config)
    ...
    if config.settings and config.settings["rust-analyzer"] then
      init_params.initializationOnptions = config.settings["rust-analyzer"]
    end
  ...
  on_attach = function(_, bufnr)
    ...
    end, { desc = "Reload current cargo workapace" })
